...
angular.module('rocketstudiowebApp')
    .factory('Bots', function ($http) {
        return {
            ...
            getProject: function (can_new, next, error) {
                $http({
                    method: 'POST',
                    url: url_server + 'getproject',
                    data: $.param({'new':can_new}),
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                }).then(function (data) {
                    if (next) { next(data); }
            ...
            newProject: function (name,path,description, author, next, error) {
                $http({
                    method: 'POST',
                    url: url_server + 'createnewproject',
                    data: $.param({ 
                        name: name, 
                        path: path , 
                    ...
                    if (next && data.data.success) { next(data.data); }
