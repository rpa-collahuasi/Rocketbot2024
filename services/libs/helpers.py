...
def setVar(datas, name, data):
    for data_ in datas:
        if str(data_['name']) == str(name).replace("}","").replace("{",""):
            if data_.get("constant", False):
                raise TypeError(f"The variable '{name}' can't be modified because it is constant")
            try:
                data_['data'] = str(data)
            except Exception as e:
                print("set data:", e)
...
def replaceByVar(vars_, command, system = [], ignore_python=False, force_str=False):
    '''
        ...
        if tmp_a:
            for v in tmp_a:
                if v in system:
                    var = system[v]
                    if force_str:
                        var = json.dumps(system[v])
                    command = command.replace("%"+str(v)+"%", var)
