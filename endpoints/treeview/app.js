...
.controller('MainController', function($timeout, $rootScope, $scope, $http){
    ...
    $scope.view_item = function(item){
        // console.log("*** item ***")
        // console.log(item)
        $scope.getChildrenCommands(item)
        try{
    ...
    $scope.getChildrenCommands = function(bot){
        console.log("*** bot ***")
        console.log(bot)
        if (bot.father == 'execRocketBotDBExpose') {
            var data = JSON.parse(bot.command);
            robotName = data.robot;
        } else {
            robotName = bot.command;
        }
        $http({
            method: 'POST',
            url: '../../getbot',
            data: $.param({ name: robotName, db:db}),
        ...
        .then(function(data){
            ...
            if (data.data.length > 0) {
                var commands = JSON.parse(data.data[0].data);
                console.log("*** commands ***")
                console.log(commands)
                $scope.bots[bot.id] = commands.project.commands;
            }
